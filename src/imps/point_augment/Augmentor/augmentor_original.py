...
class Augmentor(nn.Module):
    ...
    def forward(self, pt, global_feature, noise):
        ...
        bound_min, indices_min = torch.min(pt_aug,2)
        bound_max, indices_max = torch.max(pt_aug,2)     
        
        if all(t < -0.5 for t in bound_min.tolist()[0]): # for the moment works only for scaled scene and is hard coded
            print("entering bound min  loop")
            pt_aug = pt         
                     
        if all(t > 0.5 for t in bound_max.tolist()[0]): # for the moment works only for scaled scene and is hard coded
            print("entering bound max  loop")
            pt_aug = pt
