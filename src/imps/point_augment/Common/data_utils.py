...
class PointcloudScaleAndTranslate(object):
    ...
    def __call__(self, pc):
        ...
        for i in range(bsize):
            ...
            #print(xyz1)
            xyz1 =  torch.from_numpy(xyz1).float().cuda()
            xyz2 =  torch.from_numpy(xyz2).float().cuda()
            #print(xyz1)
            print(pc[i, :, 0:3])
            print(pc.get_device())
            pc[i, :, 0:3] = torch.mul(pc[i, :, 0:3], xyz1) + xyz2
...
class PointcloudRotate(object):
    ...
    def __call__(self, points):
        ...
        #rotation_matrix.to(device="cuda")
